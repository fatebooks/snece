...
function aleryer(a,b=0) {
    ...
    for (var x in a){
        if(b){
            var cent = '<div style="position: relative"><h1 style="text-align:center;">'+x+'</h1><button name="'+b+'" class="scened" onclick="sendscene(this)">一键发红包</button><button name="'+b+'" style="font-size: 24px;background-color: cornflowerblue ;border-radius: 8px;padding: 5px;position: absolute;right: 30px;top:1px" onclick="againscene(this)">重抽选中名单</button></div><hr><div id="aga" style="width:750px;margin:10px">';
        }else{
...
function sendscene(obj) {
    var curs = $(obj).attr('name');
    // console.log(curs);
    if(curs==current){
        layer.close(index);
        layer.alert('未录入奖项无法发送红包');
        return ;
    }
    var snecearr = new Object();
    snecearr['cid'] = c_id;
    snecearr['item']=curs;
    // var arr = [];
    // snecearr['nid'] = arr;
    console.log(snecearr);
    $.ajax({
        type: "POST",
        url: post_url,
        data: snecearr,
        success: function(msg){
            ...
            // alert(msg);
            if(msg==1){
                $(obj).attr('style','background:#ccc')
                $(obj).attr('disabled','true')
                layer.alert('红包发送成功');
            }else if(msg==2){
                layer.alert('该奖项无法重复提交');
            }else if(msg==3){
                layer.alert('该奖项未设置金额');
            }else if(msg==4) {
                layer.alert('无法重复发送红包');
            }else if(msg==10001){
                    layer.alert('未设置支付方式');
            }else if(msg==6666){
                layer.alert('意外错误');
